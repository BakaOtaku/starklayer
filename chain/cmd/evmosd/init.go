...
func displayInfo(info printInfo, mbm module.BasicManager) error {
	_, err := json.MarshalIndent(info, "", " ")
	if err != nil {
		return err
	}

	// uncomment to see genesis
	//if _, err := fmt.Fprintf(os.Stderr, "%s\n", string(sdk.MustSortJSON(out))); err != nil {
	//	return err
	//}

	fmt.Println("Address for accounts")
	Info := AccountInfos{}
	for _, b := range mbm {
		address := authtypes.NewModuleAddress(b.Name()).String()
		ethAddress := common.BytesToAddress(authtypes.NewModuleAddress(b.Name()).Bytes())
		//newAddress := sdk.AccAddress(ethAddress.Bytes())
		s := AccountInfo{
			Name:       b.Name(),
			Address:    address,
			EthAddress: ethAddress.String(),
		}
		Info.Address = append(Info.Address, s)
	}
	res, _ := json.MarshalIndent(Info, "", "    ")

	_ = ioutil.WriteFile("address.json", res, 0644)
	//if err != nil {
	//	log.Fatal(err)
	//}
...
func InitCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return displayInfo(toPrint, mbm)
		},
	}
